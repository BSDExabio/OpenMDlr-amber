...
To Run:
  ...
  Make directory for desired protein with the four letter name, put FASTA/seq file and original pdb (for comparison and
    restraints generation) inside
  For one run:
    Edit annealing_script with desired parameters
    Run
  For multiple runs:
    Edit make_runs w/desired parameters
    Make sure correct lines in annealing_script are (un)commented
